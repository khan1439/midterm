...
* t2.micro type
* ami-babc8adf AMI
* Dynacorp-Webserver-Report IAM role
...
Once the webservers are running, you should begin to test the Dynacorp website. Jerome isn't sure why the site doesn't come up when he accesses the ELB endpoint. Jerome setup a special ELB health check endpoint on each web server listening on port 80 (http) at the URL path `/health`. 
...
should generate a set of JSON files containing the output of AWS CLI commands. These files contain information describing your AWS VPC. If you run this script and then change the configuration of a resource in the VPC, you will need to run this script again to capture the changes. Note, you will need to configure the AWS CLI before you run this script.
